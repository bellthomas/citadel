...
static sgx_status_t SGX_CDECL sgx_handle_challenge_phase_1(void* pms)
{
	...
	if (_tmp_challenge_data != NULL && _len_challenge_data != 0) {
		if ( _len_challenge_data % sizeof(*_tmp_challenge_data) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_challenge_data = (uint8_t*)malloc(_len_challenge_data);
		if (_in_challenge_data == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_challenge_data, _len_challenge_data, _tmp_challenge_data, _len_challenge_data)) {
